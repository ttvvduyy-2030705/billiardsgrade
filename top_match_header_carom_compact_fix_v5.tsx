...
const TopMatchHeader = ({
  ...
  const useSingleRowCaromControls = isHandheldCaromHeader;
  ...
  const rightSlotWidth = useSingleRowCaromControls
    ? Math.round(228 * compactScale)
    : profile.isHandheldLandscape
    ? Math.round(180 * compactScale)
    : isCompact
    ? 136
    : 188;
  const switchGroupWidth = useSingleRowCaromControls
    ? Math.round(180 * compactScale)
    : profile.isHandheldLandscape
  ...
  const switchRowHeight = profile.isHandheldLandscape
    ...
    ? 20
    : 26;
  const labelFontSize = useSingleRowCaromControls
    ? clamp(Math.round(10 * compactScale), 8, 9)
    : profile.isHandheldLandscape
      ...
      <View style={[styles.rightSlot, {width: rightSlotWidth}]}>
        <View
          style={[
            ...
            useSingleRowCaromControls ? styles.switchGroupRow : undefined,
          ]}>
          {!isAnyPoolMode ? (
            <View
              style={[
                styles.switchRow,
                {minHeight: switchRowHeight},
                useSingleRowCaromControls ? styles.switchRowInline : undefined,
              ]}>
              <RNText
                numberOfLines={1}
                allowFontScaling={false}
