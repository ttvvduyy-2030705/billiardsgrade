...
const GameConsole = (props: ConsoleViewModelProps) => {
  ...
  const poolBroadcastCameraMaxHeight = useMemo(() => {
    ...
    if (useResponsiveCompact) {
      return Math.round(clamp(height * 0.31, 235, 255));
    }

    if (useTightLandscapeLayout) {
      return Math.round(clamp(height * 0.33, 245, 270));
    }

    return Math.round(clamp(height * 0.36, 260, 320));
  }, [
    height,
    useExtraCompact,
    usePoolBroadcastLayout,
    ...
    <View
      ...
      <View
        style={[
          ...
          {minHeight: cameraMinHeight},
          usePoolBroadcastLayout && poolBroadcastCameraMaxHeight
            ? {maxHeight: poolBroadcastCameraMaxHeight}
            : undefined,
        ]}>
        <Webcam
          ref={webcamRef}
