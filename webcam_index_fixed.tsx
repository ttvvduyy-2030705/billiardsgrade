...
type PreviewCameraSource = 'back' | 'front' | 'external';

const getAvailablePreviewSources = (): PreviewCameraSource[] => {
  ...
  const normalized = Array.isArray(rawSources)
    ? Array.from(
        new Set(
          rawSources.filter(
            (item: any): item is PreviewCameraSource =>
  ...
  const filtered = normalized.filter(source => {
    if (source === 'external') {
      return hasDetectedExternalWebcam();
    }
...
const resolvePreviewSource = (
  requestedSource: PreviewCameraSource | null | undefined,
  availableSources: PreviewCameraSource[],
): PreviewCameraSource => {
  if (requestedSource && availableSources.includes(requestedSource)) {
    return requestedSource;
  }

  if (availableSources.includes('back')) {
    return 'back';
  }

  if (availableSources.includes('front')) {
    return 'front';
  }

  return availableSources[0] || 'back';
};
...
const WebCam = forwardRef<WebCamHandle, WebCamComponentProps>((props, ref) => {
  ...
  const availablePreviewSources = getAvailablePreviewSources();

  const baseCameraSource = resolvePreviewSource(
    (currentCameraSource || liveSourceLock || 'back') as PreviewCameraSource,
    availablePreviewSources,
  );

  const effectiveCameraSource = resolvePreviewSource(
  ...
  const showLogoOnly =
    availablePreviewSources.length === 0 ||
    (effectiveCameraSource === 'external' && !hasDetectedExternalWebcam());
  ...
  const openFullscreen = () => {
    fullscreenSourceRef.current = effectiveCameraSource;
