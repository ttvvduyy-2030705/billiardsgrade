import {StyleSheet} from 'react-native';

import {DesignSystem} from 'theme/designSystem';

type Metrics = {
  width: number;
  height: number;
};

const createStyles = (design: DesignSystem, metrics: Metrics) => {
  const {spacing, font, radius, icon, safeArea, border, control} = design;
  const isLandscape = metrics.width > metrics.height;
  const useSidebarLayout =
    metrics.width >= 760 || (isLandscape && metrics.width >= 640);
  const isCompact = metrics.width < 560;
  const isShort = metrics.height < 620;
  const horizontalPadding = Math.max(spacing.md, safeArea.left + spacing.md);
  const categoryWidth = Math.min(268, Math.max(196, metrics.width * 0.26));
  const stackedCategoryHeight = isShort ? 108 : 124;
  const cartBarBottomOffset =
    Math.max(safeArea.bottom, spacing.sm) + spacing.md;
  const cartBarMinHeight = control.buttonHeight + spacing.xs;
  const menuListBottomPadding =
    cartBarMinHeight + cartBarBottomOffset + spacing.lg;
  const stackedCartHorizontalPadding = Math.max(
    spacing.sm,
    safeArea.left + spacing.sm,
  );
  // Match the image block used by Admin / Quản lý món.
  // The customer card uses the shared custom View component, whose default
  // alignItems is flex-start; without an explicit stretch/width the image area
  // can shrink and the image looks invisible.
  const dishImageHeight = useSidebarLayout ? 130 : 150;
  const dishCardWidth =
    metrics.width >= 980 ? '31.7%' : isCompact ? '100%' : '48.5%';
  const adminDishCardWidth = useSidebarLayout ? '32%' : '100%';

  return StyleSheet.create({
    screen: {
      flex: 1,
      backgroundColor: '#030304',
      paddingTop: safeArea.top + spacing.xs,
      paddingBottom: safeArea.bottom,
      overflow: 'hidden',
    },
    screenGlowTop: {
      position: 'absolute',
      top: -metrics.height * 0.15,
      right: -metrics.width * 0.22,
      width: metrics.width * 0.72,
      height: metrics.width * 0.72,
      borderRadius: metrics.width,
      backgroundColor: 'rgba(201,29,36,0.22)',
    },
    screenGlowBottom: {
      position: 'absolute',
      bottom: -metrics.height * 0.18,
      left: -metrics.width * 0.22,
      width: metrics.width * 0.66,
      height: metrics.width * 0.66,
      borderRadius: metrics.width,
      backgroundColor: 'rgba(255,255,255,0.045)',
    },
    topBar: {
      minHeight: control.headerHeight,
      marginHorizontal: horizontalPadding,
      paddingHorizontal: spacing.sm,
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.12)',
      backgroundColor: 'rgba(8,8,11,0.96)',
      shadowColor: '#C91D24',
      shadowOpacity: 0.24,
      shadowOffset: {width: 0, height: 12},
      shadowRadius: spacing.lg,
      elevation: 14,
      zIndex: 5,
    },
    headerSide: {
      width: useSidebarLayout ? 158 : 104,
      flexDirection: 'row',
      alignItems: 'center',
    },
    headerSideRight: {
      justifyContent: 'flex-end',
    },
    headerTitleWrap: {
      flex: 1,
      alignItems: 'center',
      justifyContent: 'center',
      paddingHorizontal: spacing.xs,
    },
    headerTitle: {
      color: '#FFFFFF',
      fontSize: font.title,
      fontWeight: '900',
      textAlign: 'center',
    },
    headerSubTitle: {
      color: 'rgba(255,255,255,0.58)',
      fontSize: font.small,
      fontWeight: '800',
      marginTop: spacing.xxs,
      textAlign: 'center',
    },
    headerAuthCenter: {
      flex: 1,
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'center',
      columnGap: spacing.sm,
    },
    headerAuthButton: {
      minHeight: control.minTouch,
      paddingHorizontal: isCompact ? spacing.sm : spacing.md,
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.22)',
      backgroundColor: '#C91D24',
      alignItems: 'center',
      justifyContent: 'center',
      shadowColor: '#FF2B2B',
      shadowOpacity: 0.26,
      shadowOffset: {width: 0, height: 8},
      shadowRadius: spacing.sm,
      elevation: 6,
    },
    headerAuthButtonSecondary: {
      minHeight: control.minTouch,
      paddingHorizontal: isCompact ? spacing.sm : spacing.md,
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.14)',
      backgroundColor: 'rgba(255,255,255,0.07)',
      alignItems: 'center',
      justifyContent: 'center',
    },
    headerAuthText: {
      color: '#FFFFFF',
      fontSize: isCompact ? font.small : font.body,
      fontWeight: '900',
    },
    backButton: {
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.14)',
      backgroundColor: 'rgba(255,255,255,0.06)',
      alignItems: 'center',
      justifyContent: 'center',
      paddingHorizontal: spacing.sm,
    },
    backText: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    headerLogo: {
      width: useSidebarLayout ? icon.xl * 2.25 : icon.xl * 1.7,
      height: useSidebarLayout ? icon.xl * 1.2 : icon.xl,
    },
    notice: {
      marginHorizontal: horizontalPadding,
      marginTop: spacing.sm,
      borderRadius: radius.lg,
      backgroundColor: 'rgba(0, 201, 94, 0.13)',
      borderWidth: border.thin,
      borderColor: 'rgba(0, 201, 94, 0.32)',
      padding: spacing.sm,
      zIndex: 4,
      rowGap: spacing.xs,
    },
    noticeText: {
      color: '#D8FFF5',
      fontSize: font.body,
      fontWeight: '800',
    },
    errorNotice: {
      backgroundColor: 'rgba(255, 40, 40, 0.13)',
      borderColor: 'rgba(255, 40, 40, 0.38)',
    },
    noticeRetryButton: {
      alignSelf: 'flex-start',
      borderRadius: radius.md,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.34)',
      backgroundColor: 'rgba(255,255,255,0.10)',
      paddingHorizontal: spacing.sm,
      paddingVertical: spacing.xs,
    },
    noticeRetryText: {
      color: '#FFFFFF',
      fontSize: font.caption,
      fontWeight: '900',
    },
    customerShell: {
      flex: 1,
      flexDirection: useSidebarLayout ? 'row' : 'column',
      paddingHorizontal: horizontalPadding,
      paddingTop: spacing.md,
      paddingBottom: spacing.sm,
      columnGap: spacing.md,
      rowGap: spacing.md,
      alignItems: 'stretch',
    },
    categoryColumn: {
      width: useSidebarLayout ? categoryWidth : '100%',
      maxHeight: useSidebarLayout ? undefined : stackedCategoryHeight,
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(8,8,12,0.96)',
      overflow: 'hidden',
      shadowColor: '#000',
      shadowOpacity: 0.5,
      shadowOffset: {width: 0, height: 14},
      shadowRadius: spacing.lg,
      elevation: 9,
    },
    categoryHeader: {
      paddingHorizontal: spacing.md,
      paddingVertical: useSidebarLayout ? spacing.md : spacing.sm,
      borderBottomWidth: border.thin,
      borderBottomColor: 'rgba(255,255,255,0.08)',
      backgroundColor: 'rgba(255,255,255,0.035)',
    },
    categoryList: {
      flexGrow: useSidebarLayout ? 1 : 0,
      flexShrink: useSidebarLayout ? 1 : 0,
    },
    categoryTitle: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
    },
    categorySubtitle: {
      color: 'rgba(255,255,255,0.50)',
      fontSize: font.small,
      fontWeight: '700',
      marginTop: spacing.xxs,
    },
    categoryListContent: {
      padding: spacing.sm,
      paddingBottom: useSidebarLayout ? spacing.lg : spacing.sm,
    },
    categoryListContentHorizontal: {
      flexDirection: 'row',
      alignItems: 'stretch',
      paddingHorizontal: spacing.sm,
      paddingVertical: spacing.sm,
      paddingBottom: spacing.sm,
      columnGap: spacing.sm,
    },
    categoryItem: {
      minHeight: control.minTouch + spacing.xs,
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.08)',
      backgroundColor: '#111116',
      paddingHorizontal: spacing.sm,
      justifyContent: 'center',
      marginBottom: useSidebarLayout ? spacing.sm : 0,
    },
    categoryItemHorizontal: {
      width: Math.max(132, Math.min(184, metrics.width * 0.34)),
      marginBottom: 0,
      marginRight: 0,
    },
    categoryItemActive: {
      backgroundColor: '#C91D24',
      borderColor: 'rgba(255,255,255,0.32)',
      shadowColor: '#FF2B2B',
      shadowOpacity: 0.34,
      shadowOffset: {width: 0, height: 10},
      shadowRadius: spacing.md,
      elevation: 10,
    },
    categoryItemText: {
      color: '#FFFFFF',
      fontSize: isCompact ? font.body : font.bodyLarge,
      fontWeight: '900',
    },
    categoryCountText: {
      color: 'rgba(255,255,255,0.64)',
      fontSize: font.small,
      fontWeight: '800',
      marginTop: spacing.xxs,
    },
    menuColumn: {
      flex: 1,
      width: '100%',
      minHeight: 0,
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(12,12,16,0.95)',
      overflow: 'hidden',
      shadowColor: '#000',
      shadowOpacity: 0.5,
      shadowOffset: {width: 0, height: 14},
      shadowRadius: spacing.lg,
      elevation: 9,
    },
    menuColumnHeader: {
      paddingHorizontal: spacing.md,
      paddingVertical: spacing.md,
      borderBottomWidth: border.thin,
      borderBottomColor: 'rgba(255,255,255,0.08)',
      backgroundColor: 'rgba(255,255,255,0.035)',
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.sm,
    },
    menuTitleBlock: {
      flex: 1,
    },
    menuColumnTitle: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
    },
    menuColumnHint: {
      color: 'rgba(255,255,255,0.56)',
      fontSize: font.small,
      fontWeight: '700',
      marginTop: spacing.xxs,
    },
    menuSearchWrap: {
      marginHorizontal: spacing.md,
      marginTop: spacing.md,
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.11)',
      backgroundColor: '#15151A',
      paddingHorizontal: spacing.sm,
      paddingTop: spacing.xs,
      paddingBottom: spacing.xs,
    },
    menuSearchInput: {
      minHeight: control.fieldHeight,
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '800',
      paddingVertical: 0,
      width: '100%',
    },
    menuCountPill: {
      color: '#FFFFFF',
      fontSize: font.small,
      fontWeight: '900',
      backgroundColor: 'rgba(201,29,36,0.84)',
      paddingHorizontal: spacing.sm,
      paddingVertical: spacing.xxs,
      borderRadius: radius.pill,
      overflow: 'hidden',
    },
    menuGridContent: {
      padding: isCompact ? spacing.sm : spacing.md,
      paddingBottom: menuListBottomPadding,
    },
    menuGrid: {
      flexDirection: 'row',
      flexWrap: 'wrap',
      justifyContent: 'space-between',
      rowGap: spacing.md,
    },
    menuGridColumnWrapper: {
      justifyContent: 'space-between',
      marginBottom: spacing.md,
    },
    dishCard: {
      width: dishCardWidth as any,
      alignItems: 'stretch',
      marginBottom: spacing.md,
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: '#15151A',
      overflow: 'hidden',
      shadowColor: '#000',
      shadowOpacity: 0.42,
      shadowOffset: {width: 0, height: 12},
      shadowRadius: spacing.md,
      elevation: 8,
    },
    adminDishCard: {
      width: adminDishCardWidth as any,
      alignItems: 'stretch',
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: '#15151A',
      overflow: 'hidden',
      shadowColor: '#000',
      shadowOpacity: 0.35,
      shadowOffset: {width: 0, height: 10},
      shadowRadius: spacing.md,
      elevation: 7,
    },
    dishImageWrap: {
      width: '100%',
      alignSelf: 'stretch',
      height: dishImageHeight,
      margin: 0,
      borderTopLeftRadius: radius.xl,
      borderTopRightRadius: radius.xl,
      overflow: 'hidden',
      backgroundColor: 'rgba(255,255,255,0.07)',
      alignItems: 'center',
      justifyContent: 'center',
      borderWidth: 0,
    },
    dishImage: {
      width: '100%',
      height: '100%',
    },
    dishImageShade: {
      position: 'absolute',
      left: 0,
      right: 0,
      bottom: 0,
      height: '42%',
      backgroundColor: 'rgba(0,0,0,0.32)',
    },
    dishCategoryLabel: {
      position: 'absolute',
      left: spacing.sm,
      bottom: spacing.sm,
      color: '#FFFFFF',
      fontSize: font.small,
      fontWeight: '900',
      backgroundColor: 'rgba(0,0,0,0.58)',
      paddingHorizontal: spacing.sm,
      paddingVertical: spacing.xxs,
      borderRadius: radius.pill,
      overflow: 'hidden',
    },
    dishBody: {
      width: '100%',
      alignSelf: 'stretch',
      padding: spacing.sm,
      paddingTop: spacing.xs,
    },
    dishName: {
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
    },
    dishDescription: {
      color: 'rgba(255,255,255,0.62)',
      fontSize: font.small,
      lineHeight: font.small * 1.42,
      marginTop: spacing.xxs,
      minHeight: font.small * 2.8,
    },
    dishFooter: {
      marginTop: spacing.sm,
      flexDirection: isCompact ? 'column' : 'row',
      alignItems: isCompact ? 'stretch' : 'center',
      justifyContent: 'space-between',
      rowGap: spacing.xs,
      columnGap: spacing.sm,
    },
    dishPrice: {
      color: '#FF4C4C',
      fontSize: font.bodyLarge,
      fontWeight: '900',
    },
    addButton: {
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      backgroundColor: '#C91D24',
      paddingHorizontal: spacing.md,
      alignItems: 'center',
      justifyContent: 'center',
      shadowColor: '#FF2B2B',
      shadowOpacity: 0.24,
      shadowOffset: {width: 0, height: 7},
      shadowRadius: spacing.sm,
      elevation: 6,
    },
    addButtonText: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    disabledText: {
      color: 'rgba(255,255,255,0.48)',
      fontSize: font.body,
      fontWeight: '900',
    },
    quantityControl: {
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,80,80,0.42)',
      backgroundColor: 'rgba(201,29,36,0.18)',
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'center',
      paddingHorizontal: spacing.xs,
      columnGap: spacing.sm,
    },
    quantityButton: {
      width: control.minTouch - spacing.xs,
      height: control.minTouch - spacing.xs,
      borderRadius: radius.pill,
      backgroundColor: '#C91D24',
      alignItems: 'center',
      justifyContent: 'center',
    },
    quantityButtonText: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
      lineHeight: font.label * 1.15,
    },
    quantityText: {
      minWidth: spacing.lg,
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
      textAlign: 'center',
    },
    quantityTextSmall: {
      minWidth: spacing.lg,
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
      textAlign: 'center',
    },
    quantityControlSmall: {
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'center',
      columnGap: spacing.xs,
      marginTop: spacing.xs,
    },
    quantityButtonSmall: {
      width: control.minTouch - spacing.sm,
      height: control.minTouch - spacing.sm,
      borderRadius: radius.pill,
      backgroundColor: '#C91D24',
      alignItems: 'center',
      justifyContent: 'center',
    },
    cartBottomBar: {
      position: 'absolute',
      left: useSidebarLayout ? undefined : stackedCartHorizontalPadding,
      right: useSidebarLayout
        ? horizontalPadding + spacing.md
        : stackedCartHorizontalPadding,
      bottom: cartBarBottomOffset,
      width: useSidebarLayout ? Math.min(430, metrics.width * 0.42) : undefined,
      minHeight: control.buttonHeight + spacing.xs,
      borderRadius: radius.xl,
      backgroundColor: '#C91D24',
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.24)',
      paddingLeft: isCompact ? spacing.sm : spacing.md,
      paddingRight: isCompact ? spacing.sm : spacing.md,
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.sm,
      shadowColor: '#FF2B2B',
      shadowOpacity: 0.45,
      shadowOffset: {width: 0, height: 12},
      shadowRadius: spacing.lg,
      elevation: 18,
      zIndex: 20,
    },
    cartBarIconWrap: {
      width: control.minTouch,
      height: control.minTouch,
      borderRadius: radius.pill,
      backgroundColor: 'rgba(0,0,0,0.22)',
      alignItems: 'center',
      justifyContent: 'center',
    },
    cartFloatingIcon: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
    },
    cartFloatingText: {
      color: '#FFFFFF',
      fontSize: isCompact ? font.body : font.bodyLarge,
      fontWeight: '900',
    },
    cartBarMain: {
      flex: 1,
    },
    cartBarSubText: {
      color: 'rgba(255,255,255,0.76)',
      fontSize: font.small,
      fontWeight: '800',
      marginTop: spacing.xxs,
    },
    cartBarTotal: {
      color: '#FFFFFF',
      fontSize: isCompact ? font.body : font.bodyLarge,
      fontWeight: '900',
    },
    cartBadge: {
      position: 'absolute',
      top: -spacing.xs,
      right: -spacing.xs,
      minWidth: spacing.lg,
      height: spacing.lg,
      paddingHorizontal: spacing.xs,
      borderRadius: radius.pill,
      backgroundColor: '#FFFFFF',
      borderWidth: border.thin,
      borderColor: '#C91D24',
      alignItems: 'center',
      justifyContent: 'center',
    },
    cartBadgeText: {
      color: '#C91D24',
      fontSize: font.small,
      fontWeight: '900',
    },
    cartModalRoot: {
      ...StyleSheet.absoluteFillObject,
      zIndex: 9999,
      elevation: 9999,
      alignItems: 'stretch',
      justifyContent: 'flex-start',
      backgroundColor: 'rgba(0,0,0,0.94)',
    },
    cartModalDimLayer: {
      ...StyleSheet.absoluteFillObject,
      backgroundColor: 'rgba(0,0,0,0.94)',
    },
    cartModalKeyboardLayer: {
      flex: 1,
      width: '100%',
      height: '100%',
      alignSelf: 'stretch',
      paddingTop: safeArea.top + spacing.sm,
      paddingBottom: Math.max(safeArea.bottom, spacing.sm),
      paddingHorizontal: 0,
      alignItems: 'stretch',
      justifyContent: 'flex-start',
      backgroundColor: 'transparent',
    },
    cartModalCard: {
      flex: 1,
      width: '100%',
      height: '100%',
      alignSelf: 'stretch',
      borderRadius: 0,
      borderWidth: 0,
      borderColor: 'transparent',
      backgroundColor: '#08080B',
      overflow: 'hidden',
      shadowColor: '#000000',
      shadowOpacity: 0,
      shadowOffset: {width: 0, height: 0},
      shadowRadius: 0,
      elevation: 0,
    },
    cartModalHeader: {
      width: '100%',
      paddingHorizontal: spacing.md,
      paddingVertical: spacing.md,
      borderBottomWidth: border.thin,
      borderBottomColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(255,255,255,0.035)',
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.md,
    },
    cartModalTitle: {
      color: '#FFFFFF',
      fontSize: font.title,
      fontWeight: '900',
    },
    cartModalSubTitle: {
      color: 'rgba(255,255,255,0.58)',
      fontSize: font.small,
      fontWeight: '800',
      marginTop: spacing.xxs,
    },
    closeButton: {
      minWidth: control.minTouch,
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.14)',
      backgroundColor: 'rgba(255,255,255,0.07)',
      paddingHorizontal: spacing.md,
      alignItems: 'center',
      justifyContent: 'center',
    },
    closeButtonText: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
    },
    cartList: {
      flex: 1,
      width: '100%',
      alignSelf: 'stretch',
    },
    cartListContent: {
      width: '100%',
      padding: spacing.md,
      paddingBottom: spacing.lg,
      rowGap: spacing.sm,
    },
    cartModalFooter: {
      width: '100%',
      alignSelf: 'stretch',
      borderTopWidth: border.thin,
      borderTopColor: 'rgba(255,255,255,0.10)',
      backgroundColor: '#101014',
      paddingHorizontal: spacing.md,
      paddingTop: spacing.sm,
      paddingBottom: spacing.md,
    },
    cartRow: {
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: '#15151A',
      padding: spacing.sm,
      marginBottom: spacing.sm,
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.md,
    },
    cartRowMain: {
      flex: 1,
    },
    cartRowRight: {
      alignItems: 'flex-end',
    },
    cartItemName: {
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
    },
    cartMeta: {
      color: 'rgba(255,255,255,0.60)',
      fontSize: font.small,
      fontWeight: '700',
      marginTop: spacing.xxs,
    },
    cartLineTotal: {
      color: '#FF4C4C',
      fontSize: font.bodyLarge,
      fontWeight: '900',
    },
    totalRow: {
      marginTop: spacing.sm,
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,80,80,0.30)',
      backgroundColor: 'rgba(201,29,36,0.14)',
      padding: spacing.md,
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
    },
    totalLabel: {
      color: 'rgba(255,255,255,0.70)',
      fontSize: font.body,
      fontWeight: '800',
    },
    totalValue: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
    },
    billSummaryCard: {
      marginTop: spacing.sm,
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,80,80,0.30)',
      backgroundColor: 'rgba(201,29,36,0.13)',
      padding: spacing.md,
      rowGap: spacing.xs,
    },
    billSummaryHeader: {
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.sm,
      marginBottom: spacing.xs,
    },
    billSummaryTitleWrap: {
      flex: 1,
    },
    billSummaryLabel: {
      color: 'rgba(255,255,255,0.60)',
      fontSize: font.small,
      fontWeight: '900',
      letterSpacing: 0.6,
    },
    billSummaryTitle: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
      marginTop: spacing.xxs,
    },
    billStatusPill: {
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.18)',
      backgroundColor: 'rgba(0,0,0,0.24)',
      paddingHorizontal: spacing.sm,
      paddingVertical: spacing.xxs,
    },
    billStatusText: {
      color: '#FFFFFF',
      fontSize: font.small,
      fontWeight: '900',
    },
    billTotalRow: {
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.sm,
    },
    billTotalLabel: {
      color: 'rgba(255,255,255,0.68)',
      fontSize: font.body,
      fontWeight: '800',
    },
    billTotalValue: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    billGrandTotalRow: {
      marginTop: spacing.xs,
      paddingTop: spacing.sm,
      borderTopWidth: border.thin,
      borderTopColor: 'rgba(255,255,255,0.12)',
    },
    billGrandTotalLabel: {
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
    },
    billGrandTotalValue: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
    },
    cartInfoSection: {
      width: '100%',
      alignSelf: 'stretch',
      marginTop: spacing.sm,
      rowGap: spacing.sm,
    },
    cartScopeCard: {
      width: '100%',
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.12)',
      backgroundColor: 'rgba(255,255,255,0.055)',
      padding: spacing.sm,
    },
    cartScopeLabel: {
      color: 'rgba(255,255,255,0.52)',
      fontSize: font.small,
      fontWeight: '900',
      letterSpacing: 0.6,
    },
    cartScopeTitle: {
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
      marginTop: spacing.xxs,
    },
    cartScopeSubTitle: {
      color: 'rgba(255,255,255,0.66)',
      fontSize: font.small,
      fontWeight: '800',
      marginTop: spacing.xxs,
    },
    tablePickerSection: {
      width: '100%',
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(255,255,255,0.04)',
      padding: spacing.sm,
    },
    tablePickerTitle: {
      color: 'rgba(255,255,255,0.68)',
      fontSize: font.small,
      fontWeight: '900',
      marginBottom: spacing.xs,
    },
    tablePickerHint: {
      color: 'rgba(255,255,255,0.56)',
      fontSize: font.small,
      fontWeight: '800',
      marginLeft: spacing.xs,
    },
    tableChipRow: {
      columnGap: spacing.xs,
      paddingRight: spacing.md,
    },
    tableChip: {
      minHeight: control.minTouch - spacing.xs,
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.16)',
      backgroundColor: '#15151A',
      paddingHorizontal: spacing.md,
      alignItems: 'center',
      justifyContent: 'center',
    },
    tableChipActive: {
      borderColor: '#FF4C4C',
      backgroundColor: 'rgba(201,29,36,0.24)',
    },
    tableChipDisabled: {
      opacity: 0.35,
    },
    tableChipText: {
      color: 'rgba(255,255,255,0.78)',
      fontSize: font.body,
      fontWeight: '900',
    },
    tableChipTextActive: {
      color: '#FFFFFF',
    },
    lockedTableCard: {
      width: '100%',
      alignSelf: 'stretch',
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(0,201,94,0.30)',
      backgroundColor: 'rgba(0,201,94,0.11)',
      padding: spacing.sm,
    },
    lockedTableLabel: {
      color: 'rgba(216,255,245,0.72)',
      fontSize: font.small,
      fontWeight: '900',
      letterSpacing: 0.6,
    },
    lockedTableNumber: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
      marginTop: spacing.xxs,
    },
    lockedTableHint: {
      color: 'rgba(216,255,245,0.72)',
      fontSize: font.small,
      fontWeight: '800',
      lineHeight: font.small * 1.42,
      marginTop: spacing.xs,
    },
    inputWrap: {
      width: '100%',
      alignSelf: 'stretch',
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.11)',
      backgroundColor: '#15151A',
      paddingHorizontal: spacing.sm,
      paddingTop: spacing.xs,
      paddingBottom: spacing.xs,
      alignItems: 'stretch',
      justifyContent: 'center',
    },
    cartInputTouchWrap: {
      minHeight: control.fieldHeight + spacing.md,
      width: '100%',
      alignSelf: 'stretch',
    },
    cartDisplayField: {
      minHeight: control.fieldHeight + spacing.md,
      width: '100%',
      alignSelf: 'stretch',
    },
    cartDisplayFieldMultiline: {
      minHeight: control.fieldHeight + spacing.lg,
    },
    cartDisplayValue: {
      width: '100%',
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
      marginTop: spacing.xxs,
    },
    cartDisplayValueMultiline: {
      fontSize: font.body,
      lineHeight: font.body * 1.35,
    },
    cartDisplayPlaceholder: {
      color: 'rgba(255,255,255,0.42)',
      fontWeight: '800',
    },
    inputWrapError: {
      borderColor: '#FF4C4C',
      backgroundColor: 'rgba(255,40,40,0.12)',
    },
    inputLabel: {
      color: 'rgba(255,255,255,0.58)',
      fontSize: font.small,
      fontWeight: '900',
      letterSpacing: 0.5,
    },
    input: {
      minHeight: control.fieldHeight,
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '800',
      paddingVertical: 0,
      width: '100%',
      flexGrow: 1,
      alignSelf: 'stretch',
    },
    textArea: {
      minHeight: control.fieldHeight + spacing.md,
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '800',
      textAlignVertical: 'top',
      paddingTop: spacing.xs,
      paddingBottom: spacing.sm,
      width: '100%',
      flexGrow: 1,
      alignSelf: 'stretch',
    },
    fieldInputOverlayRoot: {
      ...StyleSheet.absoluteFillObject,
      width: '100%',
      height: '100%',
      zIndex: 12000,
      elevation: 12000,
      justifyContent: 'flex-end',
      backgroundColor: 'rgba(0,0,0,0.34)',
    },
    fieldInputOverlayBackdrop: {
      ...StyleSheet.absoluteFillObject,
      backgroundColor: 'rgba(0,0,0,0.34)',
    },
    fieldInputKeyboardLayer: {
      ...StyleSheet.absoluteFillObject,
      width: '100%',
      height: '100%',
      alignSelf: 'stretch',
      justifyContent: 'flex-end',
      paddingHorizontal: horizontalPadding,
      paddingBottom: Math.max(safeArea.bottom, spacing.sm),
      zIndex: 12001,
      elevation: 12001,
    },
    fieldInputPanel: {
      width: '100%',
      alignSelf: 'stretch',
      zIndex: 12002,
      elevation: 12002,
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,80,80,0.36)',
      backgroundColor: '#101014',
      padding: spacing.md,
      shadowColor: '#000',
      shadowOpacity: 0.52,
      shadowOffset: {width: 0, height: -12},
      shadowRadius: spacing.lg,
    },
    fieldInputHeader: {
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.sm,
      marginBottom: spacing.sm,
    },
    fieldInputTitleWrap: {
      flex: 1,
      alignItems: 'center',
      justifyContent: 'center',
    },
    fieldInputTitle: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
      textAlign: 'center',
    },
    fieldInputSubtitle: {
      color: 'rgba(255,255,255,0.58)',
      fontSize: font.small,
      fontWeight: '800',
      marginTop: spacing.xxs,
      textAlign: 'center',
    },
    fieldInputActionButton: {
      minWidth: control.minTouch + spacing.lg,
      minHeight: control.minTouch,
      paddingHorizontal: spacing.md,
      borderRadius: radius.pill,
      alignItems: 'center',
      justifyContent: 'center',
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.14)',
      backgroundColor: 'rgba(255,255,255,0.07)',
    },
    fieldInputSaveButton: {
      backgroundColor: '#C91D24',
      borderColor: 'rgba(255,255,255,0.22)',
    },
    fieldInputCancelText: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    fieldInputSaveText: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    fieldInputTextInput: {
      width: '100%',
      minHeight: control.fieldHeight + spacing.sm,
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: '#B11226',
      backgroundColor: '#15151A',
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
      paddingHorizontal: spacing.md,
      paddingVertical: spacing.sm,
    },
    fieldInputTextArea: {
      width: '100%',
      minHeight: isShort
        ? control.fieldHeight * 1.55
        : control.fieldHeight * 2.1,
      maxHeight: metrics.height * 0.26,
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: '#B11226',
      backgroundColor: '#15151A',
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '800',
      paddingHorizontal: spacing.md,
      paddingVertical: spacing.sm,
      textAlignVertical: 'top',
    },
    fieldErrorText: {
      color: '#FF6868',
      fontSize: font.small,
      fontWeight: '900',
      marginTop: spacing.xxs,
      marginLeft: spacing.xs,
    },
    primaryButton: {
      width: '100%',
      alignSelf: 'stretch',
      minHeight: control.buttonHeight,
      borderRadius: radius.pill,
      backgroundColor: '#C91D24',
      alignItems: 'center',
      justifyContent: 'center',
      paddingHorizontal: spacing.md,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.22)',
      shadowColor: '#FF2B2B',
      shadowOpacity: 0.24,
      shadowOffset: {width: 0, height: 8},
      shadowRadius: spacing.sm,
      elevation: 6,
    },
    primaryButtonText: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    secondaryButton: {
      minHeight: control.buttonHeight,
      borderRadius: radius.pill,
      backgroundColor: 'rgba(255,255,255,0.07)',
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.14)',
      alignItems: 'center',
      justifyContent: 'center',
      paddingHorizontal: spacing.md,
    },
    secondaryButtonSmall: {
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      backgroundColor: 'rgba(255,255,255,0.07)',
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.14)',
      alignItems: 'center',
      justifyContent: 'center',
      paddingHorizontal: spacing.md,
    },
    secondaryButtonText: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    dangerButtonSmall: {
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      backgroundColor: 'rgba(255,50,50,0.16)',
      borderWidth: border.thin,
      borderColor: 'rgba(255,80,80,0.36)',
      alignItems: 'center',
      justifyContent: 'center',
      paddingHorizontal: spacing.md,
    },
    dangerButtonText: {
      color: '#FF7777',
      fontSize: font.body,
      fontWeight: '900',
    },
    emptyState: {
      width: '100%',
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(255,255,255,0.04)',
      alignItems: 'center',
      justifyContent: 'center',
      padding: spacing.xl,
      minHeight: isShort ? 150 : 220,
    },
    emptyIcon: {
      fontSize: font.titleLarge,
      marginBottom: spacing.xs,
    },
    emptyText: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
      textAlign: 'center',
    },
    emptySubText: {
      color: 'rgba(255,255,255,0.58)',
      fontSize: font.body,
      fontWeight: '700',
      marginTop: spacing.xs,
      textAlign: 'center',
    },
    emptyActionButton: {
      marginTop: spacing.md,
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      backgroundColor: '#C91D24',
      paddingHorizontal: spacing.lg,
      alignItems: 'center',
      justifyContent: 'center',
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.22)',
    },
    emptyActionText: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    adminScroll: {
      flex: 1,
    },
    adminScrollContent: {
      paddingHorizontal: horizontalPadding,
      paddingTop: spacing.md,
      paddingBottom: safeArea.bottom + spacing.xxl,
      rowGap: spacing.md,
    },
    authScrollContent: {
      paddingHorizontal: horizontalPadding,
      paddingTop: spacing.xl,
      paddingBottom: safeArea.bottom + spacing.xxl,
      alignItems: 'center',
    },
    authHero: {
      width: '100%',
      maxWidth: 760,
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,80,80,0.24)',
      backgroundColor: 'rgba(201,29,36,0.14)',
      padding: spacing.lg,
      marginBottom: spacing.md,
    },
    authCard: {
      width: '100%',
      maxWidth: 760,
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(13,13,17,0.96)',
      padding: spacing.md,
      shadowColor: '#000',
      shadowOpacity: 0.36,
      shadowOffset: {width: 0, height: 12},
      shadowRadius: spacing.lg,
      elevation: 7,
    },
    authLinkRow: {
      flexDirection: 'row',
      flexWrap: 'wrap',
      alignItems: 'center',
      justifyContent: 'space-between',
      gap: spacing.sm,
      marginTop: spacing.md,
    },
    authTextButton: {
      minHeight: control.minTouch,
      justifyContent: 'center',
    },
    authTextButtonText: {
      color: '#FF6A6A',
      fontSize: font.body,
      fontWeight: '900',
    },
    loginEyebrow: {
      color: '#FF6A6A',
      fontSize: font.small,
      fontWeight: '900',
      letterSpacing: 0.6,
      textTransform: 'uppercase',
    },
    loginTitle: {
      color: '#FFFFFF',
      fontSize: font.titleLarge,
      fontWeight: '900',
      marginTop: spacing.xs,
    },
    loginHint: {
      color: 'rgba(255,255,255,0.68)',
      fontSize: font.body,
      fontWeight: '700',
      marginTop: spacing.xs,
      lineHeight: font.body * 1.4,
    },
    dashboardHero: {
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,80,80,0.24)',
      backgroundColor: 'rgba(201,29,36,0.14)',
      padding: spacing.lg,
      marginBottom: spacing.md,
    },
    section: {
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(13,13,17,0.95)',
      padding: spacing.md,
      marginBottom: spacing.md,
      shadowColor: '#000',
      shadowOpacity: 0.36,
      shadowOffset: {width: 0, height: 12},
      shadowRadius: spacing.lg,
      elevation: 7,
    },
    sectionHeader: {
      flexDirection: 'row',
      alignItems: 'flex-start',
      justifyContent: 'space-between',
      marginBottom: spacing.md,
      columnGap: spacing.md,
    },
    sectionHeaderText: {
      flex: 1,
    },
    sectionTitle: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
    },
    sectionHint: {
      color: 'rgba(255,255,255,0.58)',
      fontSize: font.body,
      fontWeight: '700',
      marginTop: spacing.xxs,
      lineHeight: font.body * 1.35,
    },
    blockLabel: {
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
      marginTop: spacing.md,
      marginBottom: spacing.sm,
    },
    adminTabs: {
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'center',
      padding: spacing.xs,
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(255,255,255,0.045)',
      marginBottom: spacing.md,
      columnGap: spacing.xs,
    },
    adminTab: {
      flex: 1,
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      alignItems: 'center',
      justifyContent: 'center',
      paddingHorizontal: spacing.xs,
    },
    adminTabActive: {
      backgroundColor: '#C91D24',
      shadowColor: '#FF2B2B',
      shadowOpacity: 0.22,
      shadowOffset: {width: 0, height: 6},
      shadowRadius: spacing.sm,
      elevation: 5,
    },
    adminTabText: {
      color: 'rgba(255,255,255,0.72)',
      fontSize: font.body,
      fontWeight: '900',
    },
    adminTabTextActive: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    formGrid: {
      flexDirection: 'row',
      flexWrap: 'wrap',
      justifyContent: 'space-between',
      rowGap: spacing.sm,
    },
    formField: {
      width: useSidebarLayout ? '49%' : '100%',
    },
    fullField: {
      width: '100%',
      marginBottom: spacing.sm,
    },
    formActions: {
      flexDirection: 'row',
      flexWrap: 'wrap',
      alignItems: 'center',
      gap: spacing.sm,
      marginTop: spacing.md,
    },
    categorySelector: {
      flexDirection: 'row',
      flexWrap: 'wrap',
      gap: spacing.sm,
    },
    formCategoryChip: {
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      paddingHorizontal: spacing.md,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.12)',
      backgroundColor: 'rgba(255,255,255,0.06)',
      alignItems: 'center',
      justifyContent: 'center',
    },
    formCategoryChipActive: {
      backgroundColor: '#C91D24',
      borderColor: 'rgba(255,255,255,0.26)',
    },
    formCategoryChipText: {
      color: 'rgba(255,255,255,0.76)',
      fontSize: font.body,
      fontWeight: '900',
    },
    formCategoryChipTextActive: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
    availabilityRow: {
      marginTop: spacing.md,
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: 'rgba(255,255,255,0.04)',
      padding: spacing.md,
      flexDirection: 'row',
      alignItems: 'center',
      columnGap: spacing.md,
    },
    adminMenuGrid: {
      flexDirection: 'row',
      flexWrap: 'wrap',
      justifyContent: 'space-between',
      rowGap: spacing.md,
    },
    adminDishMetaRow: {
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.sm,
      marginTop: spacing.sm,
      marginBottom: spacing.sm,
    },
    adminCardActionsInline: {
      flexDirection: 'row',
      alignItems: 'center',
      gap: spacing.sm,
    },
    categoryManageList: {
      rowGap: spacing.sm,
    },
    categoryManageRow: {
      borderRadius: radius.lg,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: '#15151A',
      padding: spacing.sm,
      flexDirection: isCompact ? 'column' : 'row',
      alignItems: isCompact ? 'stretch' : 'center',
      justifyContent: 'space-between',
      rowGap: spacing.sm,
      columnGap: spacing.md,
    },
    categoryManageInfo: {
      flex: 1,
    },
    categoryManageName: {
      color: '#FFFFFF',
      fontSize: font.bodyLarge,
      fontWeight: '900',
    },
    orderCard: {
      borderRadius: radius.xl,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.10)',
      backgroundColor: '#15151A',
      padding: spacing.md,
      marginBottom: spacing.md,
    },
    orderHeader: {
      flexDirection: 'row',
      alignItems: 'center',
      justifyContent: 'space-between',
      columnGap: spacing.md,
    },
    orderTitle: {
      color: '#FFFFFF',
      fontSize: font.label,
      fontWeight: '900',
    },
    orderTime: {
      color: 'rgba(255,255,255,0.54)',
      fontSize: font.small,
      fontWeight: '800',
      marginTop: spacing.xxs,
    },
    statusPill: {
      borderRadius: radius.pill,
      paddingHorizontal: spacing.sm,
      paddingVertical: spacing.xxs,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.16)',
    },
    statusText: {
      color: '#FFFFFF',
      fontSize: font.small,
      fontWeight: '900',
    },
    orderItemsWrap: {
      marginTop: spacing.md,
      rowGap: spacing.xs,
    },
    orderItemText: {
      color: 'rgba(255,255,255,0.82)',
      fontSize: font.body,
      fontWeight: '800',
      lineHeight: font.body * 1.4,
    },
    orderNote: {
      color: '#FFD3D3',
      fontSize: font.body,
      fontWeight: '800',
      marginTop: spacing.sm,
    },
    orderFooter: {
      marginTop: spacing.md,
      alignItems: 'flex-end',
    },
    statusActions: {
      paddingTop: spacing.md,
      gap: spacing.sm,
    },
    statusButton: {
      minHeight: control.minTouch,
      borderRadius: radius.pill,
      borderWidth: border.thin,
      borderColor: 'rgba(255,255,255,0.12)',
      backgroundColor: 'rgba(255,255,255,0.06)',
      paddingHorizontal: spacing.md,
      alignItems: 'center',
      justifyContent: 'center',
    },
    statusButtonActive: {
      backgroundColor: '#C91D24',
      borderColor: 'rgba(255,255,255,0.26)',
    },
    statusButtonText: {
      color: 'rgba(255,255,255,0.76)',
      fontSize: font.body,
      fontWeight: '900',
    },
    statusButtonTextActive: {
      color: '#FFFFFF',
      fontSize: font.body,
      fontWeight: '900',
    },
  });
};

export default createStyles;
